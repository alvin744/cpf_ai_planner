...
st.set_page_config(page_title="CPF AI Retirement Planner", layout="wide")
...
st.sidebar.number_input(
    ...
    min_value=18,
    max_value=70,
    value=int(st.session_state.planner_inputs["age"]),
    step=1,
    key="age_input",
)
...
st.sidebar.number_input(
    ...
    min_value=40,
    max_value=70,
    value=int(st.session_state.planner_inputs["stop_work_age"]),
    step=1,
...
st.sidebar.number_input(
    ...
    min_value=65,
    max_value=70,
    value=int(st.session_state.planner_inputs["payout_age"]),
    step=1,
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=50000.0,
    value=float(st.session_state.planner_inputs["salary"]),
    step=100.0,
    key="salary_input",
)
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=20000.0,
    value=float(st.session_state.planner_inputs["spending"]),
    step=100.0,
    key="spending_input",
)
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=2000000.0,
    value=float(st.session_state.planner_inputs["oa_start"]),
    step=1000.0,
    key="oa_start_input",
)
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=2000000.0,
    value=float(st.session_state.planner_inputs["sa_start"]),
    step=1000.0,
    key="sa_start_input",
)
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=2000000.0,
    value=float(st.session_state.planner_inputs["ma_start"]),
    step=1000.0,
    key="ma_start_input",
)
...
st.sidebar.slider(
    ...
    min_value=0.0,
    max_value=0.10,
    value=float(st.session_state.planner_inputs["salary_growth"]),
...
st.sidebar.slider(
    ...
    min_value=0.0,
    max_value=0.10,
    value=float(st.session_state.planner_inputs["inflation"]),
    key="inflation_input",
)
...
st.sidebar.slider(
    ...
    min_value=10,
    max_value=40,
    value=int(st.session_state.planner_inputs["years_retirement"]),
...
st.sidebar.selectbox(
    ...
    index=["Standard", "Basic", "Escalating"].index(st.session_state.planner_inputs["plan"]),
    key="plan_input",
)
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=5000.0,
    value=float(st.session_state.planner_inputs["monthly_oa_housing"]),
    step=100.0,
...
st.sidebar.number_input(
    ...
    min_value=0.0,
    max_value=20000.0,
    value=float(st.session_state.planner_inputs["annual_oa_other"]),
    step=100.0,
...
if run_clicked:
    st.session_state.planner_inputs = {
        ...
        "stop_work_age": int(st.session_state.stop_work_age_input),
        "payout_age": int(st.session_state.payout_age_input),
        "salary": float(st.session_state.salary_input),
        "spending": float(st.session_state.spending_input),
        "oa_start": float(st.session_state.oa_start_input),
        "sa_start": float(st.session_state.sa_start_input),
        "ma_start": float(st.session_state.ma_start_input),
        "salary_growth": float(st.session_state.salary_growth_input),
        "inflation": float(st.session_state.inflation_input),
        "years_retirement": int(st.session_state.years_retirement_input),
        "plan": st.session_state.plan_input,
        "monthly_oa_housing": float(st.session_state.monthly_oa_housing_input),
        "annual_oa_other": float(st.session_state.annual_oa_other_input),
    }
    st.session_state.planner_has_run = True
    st.rerun()
...
age = inputs["age"]
stop_work_age = inputs["stop_work_age"]
payout_age = inputs["payout_age"]
salary = inputs["salary"]
spending = inputs["spending"]
oa_start = inputs["oa_start"]
sa_start = inputs["sa_start"]
ma_start = inputs["ma_start"]
salary_growth = inputs["salary_growth"]
inflation = inputs["inflation"]
years_retirement = inputs["years_retirement"]
plan = inputs["plan"]
monthly_oa_housing = inputs["monthly_oa_housing"]
annual_oa_other = inputs["annual_oa_other"]
...
c1.metric("Projected RA at payout age", f"${ra:,.0f}")
c2.metric("CPF LIFE payout", f"${payout:,.0f}/month")
c3.metric("Inflated spending", f"${inflated_spending:,.0f}/month")
c4.metric("Income gap", f"${gap:,.0f}")

# -----------------------------------------------------
# Estimated Retirement Sums
# -----------------------------------------------------

st.header(f"Estimated Retirement Sums for Your Cohort (Turning 55 in {sums['cohort_year']})")

s1, s2, s3 = st.columns(3)
s1.metric("Estimated BRS", f"${sums['brs']:,.0f}")
s2.metric("Estimated FRS", f"${sums['frs']:,.0f}")
s3.metric("Estimated ERS", f"${sums['ers']:,.0f}")

# -----------------------------------------------------
# Benchmark vs personal target
# -----------------------------------------------------

st.header("Benchmark vs Personal Target")
...
st.header("Early Retirement Simulator")

early_results = []
cols = st.columns(3)

for test_age, col in zip([55, 60, 65], cols):
    ...
    col.metric(f"Stop Work {test_age}", f"${payout_test:,.0f}/month")
...
# -----------------------------------------------------
# Housing vs Retirement Impact
# -----------------------------------------------------

st.header("Housing vs Retirement Impact")
